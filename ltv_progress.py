r"""
生涯顧客LTV進捗管理システム v4.2-fast（タイヤ提案アラート対応＋高速化版）
==============================
オートウェーブ 顧客育成トラッカー

【高速化内容】
 ① ランク計算ベクトル化：apply(axis=1)→pandas一括演算（50〜100倍高速）
 ② 未取引サービスベクトル化：Pythonループ→numpy一括処理
 ③ read_path キャッシュ：dataフォルダCSVを5分間メモリ保持
 ④ 登録番号_norm 事前計算：Tab0での重複apply排除
...
# ═══════════════════════════════════════════════════
# ⚡ 高速化 PATCH 1：ランク計算ベクトル化関数
#    apply(axis=1) → pandas一括演算（50〜100倍高速）
# ═══════════════════════════════════════════════════
def assign_rank_vectorized(df: pd.DataFrame) -> pd.Series:
    """
    assign_rank_customer のベクトル化版（高速）
    顧客8万人で apply(axis=1) より約50〜100倍速い。
    """
    def _col(name):
    ...
    shaken       = _col("cust_車検") >= 1
    sales        = _col("cust_自動車販売") >= 1
    multi_shaken = _col("cust_車検台数") >= 2
    oil          = _col("cust_オイル交換") >= 1
    tire         = _col("cust_タイヤ交換") >= 1
    maint        = (
        _col("cust_オイル交換") +
        _col("cust_タイヤ交換") +
        _col("cust_バッテリー交換") +
    ...
    # 優先度の低い順から代入（後が優先度高）
    rank = pd.Series("D", index=df.index)
    rank[maint]                  = "C"
    rank[shaken]                 = "B"
    rank[shaken & oil & tire]    = "A"
    rank[sales]                  = "S"
    rank[multi_shaken & sales]   = "SSS"
    return rank
...
GDRIVE_IDS_LTV = {
    ...
    "タイヤ提案履歴.csv"            : "1U0JLSq-5Ok59N4RGtvUkg0hLlDFs7gEq",
}
...
def scan_data_dir():
    ...
    txn_paths = [p for p in all_csvs if p.name not in exclude]

    return master_path, txn_paths, rsv_path, kari_path, tire_alert_path


# ═══════════════════════════════════════════════════
# ⚡ 高速化 PATCH 3：read_path に @st.cache_data 追加
#    dataフォルダCSVを5分間メモリに保持
#    タブ切替・フィルタ操作のたびの再読み込みを防止
# ═══════════════════════════════════════════════════
@st.cache_data(ttl=300, show_spinner=False)
def read_path(p, name=""):
    """Pathオブジェクトからデータフレームを読み込む（5分キャッシュ）"""
    for enc in ["utf-8-sig", "utf-8", "cp932", "shift_jis", "latin1"]:
        try:
...
_has_master = (
    ...
    or bool(GDRIVE_IDS_LTV.get("Master_Data.csv"))
)
...
# ═══════════════════════════════════════════════════
# ⚡ 高速化 PATCH 2：未取引サービス列のベクトル化
#    Pythonループ（16万回）→ numpy一括処理
# ═══════════════════════════════════════════════════
_svc_arr = np.array(all_services)
_no_txn  = (flag_matrix == 0)   # bool matrix: shape (N, n_services)
df_master["未取引サービス"] = [
    "、".join(_svc_arr[row]) for row in _no_txn
]
...
if "顧客ID" in df_master.columns:
    ...
    # ═══════════════════════════════════════════════════
    # ⚡ 高速化 PATCH 1 適用：ベクトル化ランク計算
    #    apply(assign_rank_customer, axis=1) を廃止
    # ═══════════════════════════════════════════════════
    cust_df["ランク"] = assign_rank_vectorized(cust_df)
...
has_rsv = (
    ...
    or bool(GDRIVE_IDS_LTV.get("Reservation.csv"))
)
df_rsv = None
if has_rsv:
    try:
...
# ── タイヤ提案履歴のマージ ──
# ⚡ 高速化 PATCH 4：登録番号_norm を一括計算して保存
#    Tab0 での全件 apply(normalize_plate) 再実行を防ぐ
...
with tab0:
    ...
    with col_d1:
        selected_date = st.date_input("📅 日付を選択", value=today_dt.date(), key="daily_date")
    with col_d2:
        ...
        target_store = st.selectbox(f"🏪 店舗を選択（{store_caption}）", store_options, key="daily_store")
    ...
    if not has_rsv or df_rsv is None:
        ...
        df_today = df_rsv[df_rsv["予約日"].dt.date == selected_date].copy()
        ...
        if "登録番号" in df_today.columns and "登録番号" in df_master.columns:
            ...
            # ⚡ 高速化 PATCH 4：マスター側は事前計算済みの登録番号_norm を再利用
            #    タイヤ提案マージ時に既に計算済み → 再計算不要
            df_master_tmp = df_master.copy()
            if "登録番号_norm" not in df_master_tmp.columns:
        ...
        if len(df_today) == 0:
            ...
            if len(df_rsv) > 0 and "予約日" in df_rsv.columns:
                ...
                if st.button("最新日付の来店予定を表示（デモ用）", key="demo_btn"):
                    ...
                    if "登録番号" in df_today.columns and "登録番号" in df_master.columns:
                        df_today = df_today.merge(df_master[master_cols], on="登録番号", how="left")
        ...
        if len(df_today) > 0:
            ...
            if "車検満了日" in df_today.columns:
                df_today["車検残日数"] = df_today["車検満了日"].apply(
                    lambda d: (pd.Timestamp(d) - pd.Timestamp(today_dt)).days if pd.notna(d) else None
                )
            else:
                df_today["車検残日数"] = None
            ...
            if reg_col:
                ...
                df_today["経過年数_years"] = df_today["経過年数_tuple"].apply(lambda x: x[0] if x else None)
...
""", unsafe_allow_html=True)
            ...
            if "予約時刻" in df_today.columns:
                df_today = df_today.sort_values(["予約時刻", "ランク順", "車検残日数"], ascending=[True, True, True], na_position="last")
            else:
                df_today = df_today.sort_values(["ランク順", "車検残日数"], ascending=[True, True])
            ...
            for _, row in df_today.iterrows():
                ...
                time_badge = (
                    f'<span style="background:#1e293b;color:#e2e8f0;padding:2px 8px;border-radius:6px;font-weight:700;font-size:0.82rem;">🕐 {rsv_time}</span>'
                    if rsv_time else ""
                )
                ...
                with st.expander("📝 活動記録", expanded=False):
                    if st.session_state.get(saved_key):
                        ...
                        with st.form(key=f"form_{act_key}"):
                            ...
                            staff_sel = st.selectbox("👤 担当者", options=staff_options,
                                key=f"staff_sel_{act_key}", help="サイドバーの「担当者マスター」から追加できます")

                            if is_incomplete:
                                st.warning("⚠️ 登録番号が不完全です。フルナンバーを入力してください。")
                                full_plate_input = st.text_input("登録番号（フルナンバー）",
                                    placeholder="例：千葉480て9579", help="全角・半角・スペースありでも自動正規化します")
                            else:
                                full_plate_input = ""
                            ...
                            if shaken_status_now not in ["車検予約済"]:
                                st.markdown("**🔑 車検提案**")
                                shaken_phase_sel = st.radio("車検フェーズ", options=list(SHAKEN_PHASES.values()),
                            ...
                            if untreated_svcs:
                                ...
                                for si, svc in enumerate(untreated_svcs):
                                    with svc_cols[si % 3]:
                                        svc_phases[svc] = st.radio(svc, options=list(SERVICE_PHASES.values()), index=0)

                            memo_input = st.text_input("📝 備考（任意）", placeholder="次回への申し送り等")
                            submitted = st.form_submit_button("💾 Notionに保存", type="primary", use_container_width=True)
            ...
            with btn_col2:
                if st.button("🖨️ A4接客指示シートをPDF出力", use_container_width=True, key="pdf_btn"):
                    try:
                        ...
                        RANK_BG = {
                            "SSS": colors.HexColor("#fef2f2"), "S": colors.HexColor("#fef3c7"),
                            "A": colors.HexColor("#dbeafe"), "B": colors.HexColor("#dcfce7"),
                            "C": colors.HexColor("#f1f5f9"), "D": colors.HexColor("#f8fafc"),
                        }
                        RANK_FG = {
                            "SSS": colors.HexColor("#7c2d12"), "S": colors.HexColor("#92400e"),
                            "A": colors.HexColor("#1e40af"), "B": colors.HexColor("#065f46"),
                            "C": colors.HexColor("#475569"), "D": colors.HexColor("#94a3b8"),
                        }

                        def ps(name, size, bold=False, color=colors.black, leading=None):
                            return ParagraphStyle(name, fontName=FONT, fontSize=size,
                                textColor=color, leading=leading or size * 1.4, spaceAfter=0)

                        story = []
                        story.append(Paragraph(f"接客指示シート　{today_str}　{target_store}", ps("title", 14, bold=True)))
                        ...
                        if "予約時刻" in df_pdf.columns:
                            df_pdf = df_pdf.sort_values(["予約時刻", "_ord", "車検残日数"], ascending=[True, True, True], na_position="last")
                        else:
                        ...
                        for _, row in df_pdf.iterrows():
                            ...
                            try:
                                ...
                                exp_txt = f"超過{abs(exp_days)}日" if exp_days < 0 else f"{exp_days}日後"
                            except (ValueError, TypeError):
                                exp_txt = "─"
                            ...
                            shaken_label_map = {
                                "車検予約済": "✅車検予約済", "仮予約あり": "△車検仮予約",
                                "予約なし": "×車検予約なし", "相談中": "?車検相談中",
                            }
                            shaken_label_pdf = shaken_label_map.get(shaken_st, shaken_st)
                            shaken_color_pdf = {
                                "車検予約済": colors.HexColor("#166534"), "仮予約あり": colors.HexColor("#854d0e"),
                                "予約なし": colors.HexColor("#991b1b"), "相談中": colors.HexColor("#075985"),
                            ...
                            status_svc_data = [[
                                Paragraph(shaken_label_pdf, ParagraphStyle("sh", fontName=FONT, fontSize=8, textColor=shaken_color_pdf, leading=11)),
                                Paragraph(f"本日：{svc_today}　{app_txt}", ParagraphStyle("st2", fontName=FONT, fontSize=7.5, textColor=colors.HexColor("#475569"), leading=11)),
                                Paragraph(f"店舗：{store_name}", ParagraphStyle("st3", fontName=FONT, fontSize=7.5, textColor=colors.HexColor("#475569"), leading=11)),
                                Paragraph("", ParagraphStyle("blank", fontName=FONT, fontSize=7, leading=10)),
                            ]]
                            svc_status_data = [[
                                Paragraph(f"取引状況：{svc_status_txt}", ParagraphStyle("ss", fontName=FONT, fontSize=7.5, textColor=colors.HexColor("#334155"), leading=11)),
                            ]]
                            detail_data = [[
                                Paragraph(f"未取引：{untreated}", ParagraphStyle("un", fontName=FONT, fontSize=8, textColor=colors.HexColor("#dc2626"), leading=11)),
                            ]]
                            ...
                            if (0 if pd.isna(_tf2) else int(_tf2)) == 1 and tire_alert_pdf:
                                ...
                                tire_data = [[Paragraph(" / ".join(tire_parts), ParagraphStyle("ta", fontName=FONT, fontSize=7.5, textColor=colors.HexColor("#9a3412"), leading=10))]]
                            ...
                            if pdf_prev_log:
                                ...
                                prev_log_data = [[Paragraph(f"[前回提案] {pl_date} 担当:{pl_staff} / {pl_item} -> {pl_phase}{fu_mark}",
                                    ParagraphStyle("pl", fontName=FONT, fontSize=7.5,
                                        textColor=colors.HexColor("#854d0e") if is_fu else colors.HexColor("#065f46"), leading=10))]]

                            biko_data = []
                            if biko_pdf:
                                biko_data = [[Paragraph(f"備考：{biko_pdf[:80]}{'...' if len(biko_pdf)>80 else ''}",
                                    ParagraphStyle("bk", fontName=FONT, fontSize=7.5, textColor=colors.HexColor("#475569"), leading=10))]]

                            action_data = [[Paragraph(f"【接客指示】{action}",
                                ParagraphStyle("ac", fontName=FONT, fontSize=8.5, textColor=colors.HexColor("#1e3a5f"), leading=12))]]
                            ...
                            card = Table(all_rows, colWidths=[22*mm, 45*mm, 55*mm, 50*mm] if len(header_data[0])==4 else [172*mm])
                            style_cmds = [
                                ("FONTNAME", (0,0), (-1,-1), FONT),
...
with tab1:
    ...
    st.bar_chart(df_store_summary, color=["#d97706", "#2563eb", "#059669", "#94a3b8", "#e2e8f0"])
    ...
    for st_name in stores:
        ...
        s_n = (ds["ランク"] == "S").sum()
        tbl_rows.append({
            "店舗": st_name, "管理台数": n, "Sランク": s_n,
            "Aランク": (ds["ランク"] == "A").sum(), "Bランク": (ds["ランク"] == "B").sum(),
            "Cランク": (ds["ランク"] == "C").sum(), "Dランク": (ds["ランク"] == "D").sum(),
            "S率": f"{s_n/n*100:.1f}%",
    ...
    rank_def_rows = [
        {"ランク": "🏆 SSS", "意味": "完全囲い込み", "判定条件": "複数台で車検取引あり かつ 自動車販売あり", "対応方針": "VIP対応。次の1台・家族の車を必ず確認。担当者固定"},
        {"ランク": "🥇 S",   "意味": "乗り換え獲得", "判定条件": "車検あり かつ 自動車販売あり",               "対応方針": "2台目・家族の車検確認。来店習慣をさらに強化する"},
        {"ランク": "🥈 A",   "意味": "来店習慣定着", "判定条件": "車検あり かつ オイル交換あり かつ タイヤ交換あり", "対応方針": "乗り換え提案が最重要。車齢・走行距離を確認してタイミングを探る"},
        {"ランク": "🥉 B",   "意味": "車検定着",     "判定条件": "車検あり（A未満）",                           "対応方針": "オイル＋タイヤのセット化が目標。次回来店予約を今日取る"},
        {"ランク": "C",       "意味": "車検未獲得",   "判定条件": "車検なし・メンテ来店あり",                    "対応方針": "車検獲得が最優先。信頼関係を築きながら必ず一声かける"},
        {"ランク": "D",       "意味": "ほぼ未取引",   "判定条件": "メンテ来店もほぼなし",                        "対応方針": "次回来店予約を今日取る。まず接触頻度を上げることが最優先"},
    ]
    st.caption("※ ランクは顧客ID単位で判定します（同一顧客の複数台を合算）")
    st.dataframe(pd.DataFrame(rank_def_rows), use_container_width=True, hide_index=True)
    ...
    if st.button("📄 ダッシュボードをPDF出力（A4印刷用）", key="dash_pdf_btn", use_container_width=True):
        try:
            ...
            def _p(text, size=9, color=rl_colors.black, bold=False, leading=None):
                return Paragraph(str(text), ParagraphStyle("x", fontName=FONT, fontSize=size,
            ...
            story.append(_p(f"LTV進捗ダッシュボード　出力日：{datetime.today().strftime('%Y/%m/%d %H:%M')}", size=13, color=rl_colors.HexColor("#0a1628")))
            ...
            kpi_data = [
                ...
                [f"{total:,}台", f"{s_cnt_pdf:,}台", f"{a_cnt_pdf+b_cnt_pdf:,}台", f"{cd_cnt_pdf:,}台", f"{rsv_cnt_pdf:,}台"],
            ...
            rank_hdr = [_p(h,8) for h in ["ランク","台数","割合","意味","判定条件","平均取引数","予約取得率"]]
            rank_rows_pdf = [rank_hdr]
            meanings = {"SSS":"完全囲い込み","S":"乗り換え獲得","A":"来店習慣定着","B":"車検定着","C":"車検未獲得","D":"ほぼ未取引"}
            conds = {"SSS":"複数台車検＋車販","S":"車検＋車販","A":"車検＋オイル＋タイヤ","B":"車検あり（A未満）","C":"車検なし・メンテ来店","D":"ほぼ未取引"}
            for r in ["SSS","S","A","B","C","D"]:
                dr = df_master[df_master["ランク"]==r]
                if len(dr) == 0: continue
                fg = rl_colors.HexColor(rank_colors_pdf.get(r, "#374151"))
                rank_rows_pdf.append([
                    _p(f"{r}ランク", 9, fg), _p(f"{len(dr):,}人", 9), _p(f"{len(dr)/total*100:.1f}%", 9),
                    _p(meanings.get(r,""), 8), _p(conds.get(r,""), 8),
                    _p(f"{dr['取引サービス数'].mean():.1f}", 9),
                    _p(f"{(dr['予約件数']>0).sum()/len(dr)*100:.1f}%", 9),
                ])
            rank_tbl = Table(rank_rows_pdf, colWidths=[20*mm, 18*mm, 14*mm, 22*mm, 22*mm, 22*mm, 22*mm])
            ...
            store_tbl = Table(store_rows_pdf, colWidths=[30*mm, 18*mm, 12*mm, 12*mm, 12*mm, 12*mm, 12*mm, 14*mm, 18*mm])
            ...
            for s in all_services:
                ...
                svc_rows_pdf.append([
                    ...
                    _p(f"{done/total*100:.1f}%", 8), _p(f"¥{pot/1e6:.1f}M", 8),
                ])
            ...
            story.append(_p(f"オートウェーブ 生涯顧客LTV管理システム　出力日時：{datetime.today().strftime('%Y/%m/%d %H:%M')}",
                size=7, color=rl_colors.HexColor("#94a3b8")))

            doc.build(story)
            pdf_bytes = pdf_buf.getvalue()
            st.download_button("📄 ダッシュボードPDFをダウンロード", pdf_bytes,
...
with tab3:
    ...
    df_cs = df_master[
        (df_master["ランク"].isin(cs_rank)) & (df_master["入庫店舗ID"].isin(cs_store))
    ]

    if len(all_services) < 2:
        st.info("クロスセル分析には2種類以上の取引CSVが必要です。")
    else:
    ...
    if len(all_services) < 2:
        ...
        for svc_a in all_services:
            ...
            for svc_b in all_services:
                ...
                opps.append({
                    "起点（取引済）": svc_a, "提案（未取引）": svc_b,
                    "対象台数": untreated, "潜在売上": f"¥{potential/1e6:.1f}M",
                })
        if opps:
    ...
    if has_rsv and df_rsv is not None and "サービス種別" in df_rsv.columns:
        st.markdown('<div class="sec">サービス別 予約取得状況</div>', unsafe_allow_html=True)
        rsv_by_svc = df_rsv["サービス種別"].value_counts().rename("予約件数")
        svc_base = pd.DataFrame({
            "サービス": all_services,
            "取引済み台数": [df_master[f"取引_{s}"].sum() for s in all_services],
        })
        rsv_m = svc_base.merge(rsv_by_svc.reset_index().rename(columns={"index":"サービス","サービス種別":"サービス"}),
            on="サービス", how="left").fillna(0)
...
with tab5:
    ...
    if has_rsv and df_rsv is not None:
        ...
        if "登録番号" in df_rsv_f.columns and "登録番号" in df_master.columns:
            df_rsv_f = df_rsv_f.merge(df_master[["登録番号","入庫店舗ID","ランク","取引サービス数"]], on="登録番号", how="left")
        ...
        if len(df_rsv_f) > 0:
            st.download_button("📥 予約リストCSV", to_csv(df_rsv_f), "reservation_filtered.csv", "text/csv", use_container_width=True)
...
""", unsafe_allow_html=True)
    ...
    for s in all_services:
        ...
        pot_rows.append({
            "サービス": s, "取引済み台数": total - untreated, "未取引台数": untreated,
            "未取引率": f"{untreated/total*100:.1f}%", "潜在売上推計": f"¥{pot/1e6:.1f}M",
        })
    if pot_rows:
        st.dataframe(pd.DataFrame(pot_rows).sort_values("未取引台数", ascending=False), use_container_width=True, hide_index=True)
...
<div style="background:{bg};border-radius:8px;padding:0.6rem;text-align:center;margin:0.2rem 0;">
  ...
  <div style="font-size:0.7rem;color:{fg};">件</div>
            ...
            for svc in [s for s in display_svcs if s != SHAKEN_SVC]:
                ...
                bar_html = ""
                for n, color, label in [(n_dame,"#e2e8f0","提案できなかった"),(n_teian,"#93c5fd","提案した"),(n_kyomi,"#fde68a","興味あり"),(n_ureta,"#86efac","売れた")]:
                    if n > 0 and total_svc > 0:
                        pct = n / total_svc * 100
                        bar_html += f'<div style="width:{pct:.1f}%;background:{color};height:24px;display:inline-block;vertical-align:middle;"></div>'
  ...
  <div style="display:flex;gap:1rem;font-size:0.7rem;color:#64748b;margin-top:0.2rem;">
    ...
    <span>■ <span style="color:#22c55e;">成約{n_ureta}</span></span>
        ...
        if "ランク" in df_f.columns:
            ...
            rank_kpi["転換率"] = (rank_kpi["興味あり"] / rank_kpi["提案件数"].replace(0,1) * 100).round(1).astype(str) + "%"
            rank_kpi = rank_kpi[rank_kpi["提案件数"] > 0]
            rank_order_kpi = ["SSS","S","A","B","C","D"]
            rank_kpi["ランク順"] = rank_kpi["ランク"].map({r:i for i,r in enumerate(rank_order_kpi)}).fillna(9)
        ...
        if len(df_f) > 0:
            for staff_n in sorted(df_f["担当者"].dropna().unique()):
                ...
                with st.expander(f"👤 {staff_n}　提案{total_p}件 / 興味あり{total_k}件 / 成約{total_s}件", expanded=False):
                    df_st_svc = df_st.groupby("提案項目").agg(
                        提案=("フェーズ番号", lambda x: (x >= 2).sum()),
                        興味あり=("フェーズ番号", lambda x: (x >= 3).sum()),
                        成約=("フェーズ番号", lambda x: (x >= 5).sum()),
                    ).reset_index()
                    df_st_svc["転換率"] = (df_st_svc["興味あり"] / df_st_svc["提案"].replace(0,1) * 100).round(1).astype(str) + "%"
        ...
        if len(df_followup) > 0:
            ...
            df_followup["満了まで(日)"] = (df_followup["車検満了日_dt"] - today_dt).dt.days
            ...
            show_cols = ["記録日","顧客ID","車両ID","登録番号","担当者","フェーズ","車検満了日","満了まで(日)","アクション","備考"]
            show_cols = [c for c in show_cols if c in df_followup.columns]
            st.dataframe(df_followup[show_cols].sort_values("満了まで(日)"), use_container_width=True, hide_index=True)
            buf_fu = BytesIO()
            df_followup[show_cols].to_csv(buf_fu, index=False, encoding="utf-8-sig")
            st.download_button("📥 フォローアップリストCSVダウンロード（予約システム取込用）",
                buf_fu.getvalue(), f"followup_shaken_{datetime.today().strftime('%Y%m%d')}.csv",
                "text/csv", use_container_width=True)
        else:
        ...
        df_all_kyomi = df_kpi[df_kpi["フェーズ_clean"].isin(["興味あり","仮予約"])].copy()
        if len(df_all_kyomi) > 0:
            kyomi_cols = ["記録日","顧客ID","車両ID","登録番号","担当者","提案項目","フェーズ","備考"]
            kyomi_cols = [c for c in kyomi_cols if c in df_all_kyomi.columns]
            st.dataframe(df_all_kyomi[kyomi_cols].sort_values("記録日", ascending=False), use_container_width=True, hide_index=True)
            buf_k = BytesIO()
            df_all_kyomi[kyomi_cols].to_csv(buf_k, index=False, encoding="utf-8-sig")
            st.download_button("📥 全サービス興味ありリストCSVダウンロード", buf_k.getvalue(),
                f"kyomi_all_{datetime.today().strftime('%Y%m%d')}.csv", "text/csv", use_container_width=True)
        else:
...
<div style="text-align:center;font-size:0.72rem;color:#94a3b8;padding:0.5rem 0;">
    生涯顧客LTV進捗管理システム v4.2-fast ─ オートウェーブ ─ セッション完結型・データ永続化なし
</div>
""", unsafe_allow_html=True)
